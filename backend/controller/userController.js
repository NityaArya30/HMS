...
export const login  = catchAsyncErrors(async(req, res, next)=> {
    ...
    const user = await User.findOne({email}).select("+password") //jis jis ka schema m select false h vo lene hein yha p
    if(!user){
...
export const addNewAdmin = catchAsyncErrors(async(req, res, next) => {
    ...
    if(isRegistered) {
        return next(new ErrorHandler(` ${isRegistered.role} already exists with provided email!`));
    }
...
export const addNewDoctor = catchAsyncErrors(async(req, res, next) => {
    ...
    if( !firstName || 
        ...
        if(!cloudinaryResponse || cloudinaryResponse.error) {
            ...
            return next(new ErrorHandler("Filed to upload Doctor Avatar"), 500);
        }
